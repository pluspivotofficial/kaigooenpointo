...
## 総応募シートへのCSV追記
- `appendLatestTotalCsv()` … `TOTAL_FOLDER_ID` の最新CSVを `TOTAL_SHEET_ID` の末尾へ追記（列名でマッピング）。
- 二重追記防止に処理済みファイルIDを記録。**フォルダには新しく吐き出したCSVのみ**を置く運用にすること。
- 自動化する場合はこの関数を毎日トリガーに登録し、`runDailyAggregation` の前に実行。
